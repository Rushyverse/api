...
class LocaleGUITest : AbstractKoinTest() {
    ...
    inner class Register {
        ...
        fun `should register if not already registered`() = runTest {
            val gui = LocaleNonFillGUI(plugin, serverMock)
    ...
    inner class Open {
        ...
        fun `should fill the inventory in the same thread if no suspend operation`() {
            runBlocking {
                ...
                val gui = LocaleFillGUI(plugin, serverMock)
                gui.register()
                val (player, client) = registerPlayer()
                ...
                LocaleFillGUI.EXPECTED_INV.forEachIndexed { index, item ->
                    content[index] shouldBe item
                }

                for (i in LocaleFillGUI.EXPECTED_INV.size until content.size) {
        ...
        fun `should fill the inventory in the other thread after suspend operation`() {
            runBlocking {
                ...
                val gui = LocaleFillGUI(plugin, serverMock, delay)
                gui.register()
                val (player, client) = registerPlayer()
    ...
    inner class Close {
        ...
        fun `should close all inventories and remove all viewers`() = runTest {
            val gui = LocaleNonFillGUI(plugin, serverMock, InventoryType.BREWING)
            gui.register()
    ...
    inner class CloseForClient {
        ...
        fun `should not stop loading the inventory if the client is viewing the GUI`(closeInventory: Boolean) {
            runBlocking {
                val gui = LocaleFillGUI(plugin, serverMock, 10.minutes, InventoryType.ENDER_CHEST)
                gui.register()
                val (player, client) = registerPlayer()
